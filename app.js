...
R.order.forEach(k=>{
 const x=R.months[k];
 DATA.months[k]={...x,start:x.start,end:x.end,
...
function creditTrend(k){const i=order.indexOf(k);if(i<1)return'No prior month available';const d=creditTotal(k)-creditTotal(order[i-1]);if(Math.abs(d)<.005)return'No change in known balances from previous month';return`<span class="${d<0?'positive':'negative'}">${d<0?'−':'+'}${fmt2(Math.abs(d))} ${d<0?'decrease':'increase'}</span> in known balances from previous month`}
...
function accounts(){const x=D[latest],auto=AUTOLOAN.at(-1).balance;return`${topReviewBar()}<div class="two"><div class="card"><h2>Cash accounts</h2>${Object.entries(x.endAccounts).map(([n,v])=>`<div class="accountrow"><span>${n}<div class="note">${trendText(accountDelta(n),x.label)}</div></span><strong>${fmt2(v)}</strong></div>`).join('')}</div><div class="card"><h2>Investments</h2><div class="accountrow"><span>401(k)<div class="note">As of ${K401.at(-1).date}</div></span><strong>${fmt2(K401.at(-1).balance)}</strong></div><div class="accountrow"><span>Roth IRA<div class="note">${trendText(ROTH.at(-1).balance-ROTH.at(-2).balance,ROTH.at(-1).date)}</div></span><strong>${fmt2(ROTH.at(-1).balance)}</strong></div></div><div class="card"><h2>Known card balances</h2>${creditCards(latest).map(c=>`<div class="accountrow"><span>${esc(c.name)}<div class="note">As of ${c.asof}${c.status==='inferred'?' · inferred':''}</div></span><strong>${fmt2(c.balance)}</strong></div>`).join('')}</div><div class="card"><h2>Auto loan</h2><div class="accountrow"><span>Remaining balance</span><strong>${fmt2(auto)}</strong></div><div class="note">Original ${fmt2(AUTO_ORIGINAL)} · ${fmt2(AUTO_PAYMENT)}/month · roughly ${Math.ceil(auto/AUTO_PAYMENT)} payments at ${fmt2(AUTO_PAYMENT)} (ignores interest)</div></div></div>`}
...
function openDetail(action,filter){const m=$('#modal'),title=$('#modalTitle'),sub=$('#modalSub'),body=$('#modalBody');if(action==='creditMonth'){const a=creditCards(sel.value);title.textContent='Known card balances';sub.textContent=D[sel.value].label;body.innerHTML=`<table><thead><tr><th>Credit card</th><th>Statement date</th><th class="amount">Balance</th></tr></thead><tbody>${a.map(c=>`<tr><td>${esc(c.name)}</td><td>${c.asof}${c.status==='inferred'?' · inferred':''}</td><td class="amount">${fmt2(c.balance)}</td></tr>`).join('')}</tbody></table><div class="modaltotal"><span>Known balance total</span><span>${fmt2(creditTotal(sel.value))}</span></div>`;m.showModal();return}
 if(action==='autoLoan'){title.textContent='Auto loan';sub.textContent='Balance history';body.innerHTML=`<div class="grid4 mini">${card('Original',fmt2(AUTO_ORIGINAL))}${card('Remaining',fmt2(AUTOLOAN.at(-1).balance))}${card('Monthly payment',fmt2(AUTO_PAYMENT))}${card('Rough payments','~'+Math.ceil(AUTOLOAN.at(-1).balance/AUTO_PAYMENT),'Ignores interest')}</div><table><thead><tr><th>Date</th><th class="amount">Balance</th></tr></thead><tbody>${AUTOLOAN.map(r=>`<tr><td>${r.date}</td><td class="amount">${fmt2(r.balance)}</td></tr>`).join('')}</tbody></table>`;m.showModal();return}
