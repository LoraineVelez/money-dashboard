...
try{
 ...
 await nav(page,'Upcoming').click();assert.match(await page.locator('#app').innerText(),/Upcoming ·/);assert.ok(await page.locator('.calendargrid').count(),'upcoming calendar missing');assert.ok(await page.locator('.billrow').count()>0,'no recurring bills detected from statement history');
 const due=page.locator('.dueinput').first(),paid=page.locator('.paidbtn').first();const oldDue=await due.inputValue();const oldDay=Number(oldDue.slice(-2)),newDay=oldDay===15?16:15;const newDue=oldDue.slice(0,-2)+String(newDay).padStart(2,'0');await due.fill(newDue);await due.dispatchEvent('change');assert.equal(Number((await page.locator('.dueinput').first().inputValue()).slice(-2)),newDay,'due date edit did not persist');await paid.click();assert.match(await page.locator('.paidbtn').first().innerText(),/Undo paid/);await page.locator('.paidbtn').first().click();assert.match(await page.locator('.paidbtn').first().innerText(),/Mark paid/);
 ...
 console.log('Browser smoke passed: recurring Upcoming calendar, editable due dates, paid state, months, desktop/mobile, navigation, filters, charts, editing, undo/redo, rules, card/loan details, and graceful data failure.');
